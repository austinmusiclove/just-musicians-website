...
function listings_search_algo_orderby($orderby, $query) {
    ...
    if (!empty($search_term)) {
        return "
            MATCH({$wpdb->posts}.post_title, {$wpdb->posts}.post_content) AGAINST ('{$escaped}' IN BOOLEAN MODE) DESC,
            CAST(rank_meta.meta_value AS UNSIGNED) DESC,
            {$wpdb->posts}.ID ASC
        ";
    }

    return "CAST(rank_meta.meta_value AS UNSIGNED) DESC, {$wpdb->posts}.ID ASC";
}
