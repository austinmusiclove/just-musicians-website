...
<div class="lg:container min-h-[500px]" data-testid="listing-form-alpine"
    x-data="{
        ...
        postStatus:             '<?php echo $listing_data['post_status'] ?? 'draft'; ?>',
        pName:                  '<?php echo clean_str_for_doublequotes($listing_data["name"]        ?? ''); ?>',
        pDescription:           '<?php echo clean_str_for_doublequotes($listing_data["description"] ?? ''); ?>',
        pCity:                  '<?php echo clean_str_for_doublequotes($listing_data["city"]        ?? ''); ?>',
        pState:                 '<?php echo clean_str_for_doublequotes($listing_data["state"]       ?? ''); ?>',
        pZipCode:               '<?php echo $listing_data["zip_code"] ?? ''; ?>',
        zipCodeInput:           '<?php if ($listing_data) { echo $listing_data["city"] . ', ' . $listing_data["state"] . ' ' . $listing_data["zip_code"]; } ?>',
        fullLocation:           '<?php if ($listing_data) { echo $listing_data["city"] . ', ' . $listing_data["state"] . ' ' . $listing_data["zip_code"]; } ?>',
        pBio:                   '<?php echo clean_str_for_doublequotes($listing_data["bio"] ?? ''); ?>',
        pEmail:                 '<?php echo $listing_data["email"]                  ?? ''; ?>',
        pPhone:                 '<?php echo $listing_data["phone"]                  ?? ''; ?>',
        pInstagramHandle:       '<?php echo $listing_data["instagram_handle"]       ?? ''; ?>',
        pInstagramUrl:          '<?php echo $listing_data["instagram_url"]          ?? ''; ?>',
        pTiktokHandle:          '<?php echo $listing_data["tiktok_handle"]          ?? ''; ?>',
        pTiktokUrl:             '<?php echo $listing_data["tiktok_url"]             ?? ''; ?>',
        pXHandle:               '<?php echo $listing_data["x_handle"]               ?? ''; ?>',
        pXUrl:                  '<?php echo $listing_data["x_url"]                  ?? ''; ?>',
        pWebsite:               '<?php echo $listing_data["website"]                ?? ''; ?>',
        pFacebookUrl:           '<?php echo $listing_data["facebook_url"]           ?? ''; ?>',
        pYoutubeUrl:            '<?php echo $listing_data["youtube_url"]            ?? ''; ?>',
        pBandcampUrl:           '<?php echo $listing_data["bandcamp_url"]           ?? ''; ?>',
        pSpotifyArtistUrl:      '<?php echo $listing_data["spotify_artist_url"]     ?? ''; ?>',
        pSpotifyArtistId:       '<?php echo $listing_data["spotify_artist_id"]      ?? ''; ?>',
        pAppleMusicArtistUrl:   '<?php echo $listing_data["apple_music_artist_url"] ?? ''; ?>',
        pSoundcloudUrl:         '<?php echo $listing_data["soundcloud_url"]         ?? ''; ?>',
        pThumbnailSrc:          '<?php echo $listing_data['thumbnail_url']          ?? $ph_thumbnail; ?>',
        ensembleSizeCheckboxes:  <?php echo clean_arr_for_doublequotes($listing_data["ensemble_size"]      ?? []); ?>,
        categoriesCheckboxes:    <?php echo clean_arr_for_doublequotes($listing_data["mcategory"]          ?? []); ?>,
        genresCheckboxes:        <?php echo clean_arr_for_doublequotes($listing_data["genre"]              ?? []); ?>,
        subgenresCheckboxes:     <?php echo clean_arr_for_doublequotes($listing_data["subgenre"]           ?? []); ?>,
        instCheckboxes:          <?php echo clean_arr_for_doublequotes($listing_data["instrumentation"]    ?? []); ?>,
        settingsCheckboxes:      <?php echo clean_arr_for_doublequotes($listing_data["setting"]            ?? []); ?>,
        keywords:                <?php echo clean_arr_for_doublequotes($listing_data["keyword"]            ?? []); ?>,
        allVenuesPlayed:         <?php echo clean_arr_for_doublequotes($all_venues_played                  ?? []); ?>,
        verifiedVenueIds:        <?php echo clean_arr_for_doublequotes($verified_venue_ids                 ?? []); ?>,
        unverifiedVenueIds:      <?php echo clean_arr_for_doublequotes($unverified_venue_ids               ?? []); ?>,
        youtubeVideoData:        <?php echo clean_arr_for_doublequotes($listing_data["youtube_video_data"] ?? []); ?>,
        orderedImageData: {
            'cover_image': [
                {
                    'image_id':      'cover_image',
                    'attachment_id': '<?php echo $listing_data['thumbnail_id']       ?? ''; ?>',
                    'url':           '<?php echo $listing_data['thumbnail_url']      ?? ''; ?>',
                    'filename':      '<?php echo $listing_data['thumbnail_filename'] ?? ''; ?>',
                    'mediatags':      <?php echo clean_arr_for_doublequotes($listing_data["thumbnail_terms"] ?? []); ?>,
                    'loading':       false,
                    'worker':        null,
                },
            ],
            'listing_images':         <?php echo clean_arr_for_doublequotes($listing_data["listing_images_data"] ?? []); ?>,
            'stage_plots':            <?php echo clean_arr_for_doublequotes($listing_data["stage_plots_data"]    ?? []); ?>,
        },
