...
if ( is_wp_error($result) ) {
    ...
    // For updating account settings from review popup
    if ($args['is_popup']) {
        echo '<span x-init="_handleUpdateAccountSettingsError(\'' . $message . '\')"></span>';
    }
    exit;
} else {
    ...
    echo '<span x-init="$dispatch(\'success-toast\', { \'message\': \'Account settings updated successfully\'})"></span>';

    // For updating account settings from review popup
    if ($args['is_popup']) {
        echo '<span x-init="_handleUpdateAccountSettingsSuccess()"></span>';
    }
    exit;
}
