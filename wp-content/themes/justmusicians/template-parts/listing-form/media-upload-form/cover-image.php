<div class="flex flex-col h-full p-2 overflow-hidden">
    ...
    <div class="hidden">
        ...
        <input name="cover_image_meta" type="hidden" x-bind:value="JSON.stringify(imageData['cover_image']['cover_image'])">
    </div>
    ...
    <!-- State 1 -->
    <div class="flex flex-col items-center justify-center grow text-center px-8" x-show="!imageData['cover_image']['cover_image']?.url" x-cloak>
    ...
    <!-- State 2 -->
    <div class="overflow-hidden flex items-center justify-center grow mt-4 pb-2" x-show="imageData['cover_image']['cover_image']?.url" x-cloak>
        <div class="grid grid-cols-2 gap-2">
            <div>
                <div class="aspect-4/3 w-full max-w-full max-h-full">
                    <img class="h-full object-cover" x-bind:src="imageData['cover_image']['cover_image']?.url">
                </div>
            </div>
            <div class="flex items-center flex-col justify-center gap-4 px-4">
                ...
                <div class="text-center text-grey" x-text="imageData['cover_image']['cover_image']?.filename"></div>
                <div class="flex gap-2">
                    <button type="button" class="w-fit rounded text-14 border border-black/40 group flex items-center font-bold py-1 px-2 hover:border-black disabled:bg-grey disabled:text-white"
                        x-on:click="$refs.cropperDisplay.src = imageData['cover_image']['cover_image']?.url; _initCropper($refs.cropperDisplay, 'cover_image', 'cover_image'); showImageEditPopup = true;"
                    >Edit</button>
                    <button type="button" class="w-fit rounded text-14 border border-black/40 group flex items-center font-bold py-1 px-2 hover:border-black disabled:bg-grey disabled:text-white"
                ...
                <div class="tags flex flex-wrap gap-1 min-w-0 max-h-24 overflow-scroll">
                    <template x-for="tag in imageData['cover_image']['cover_image']?.mediatags" :key="tag">
