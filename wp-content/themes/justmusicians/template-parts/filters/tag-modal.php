<div data-popup="filter" class="popup-wrapper w-screen h-screen fixed top-0 left-0 z-50 flex items-center justify-center" x-show="<?php echo $args['x-show']; ?>" x-transition x-cloak>
    ...
    <div class="bg-white relative w-full h-full pt-20 px-4 sm:px-8 md:px-20 pb-20 md:max-h-[550px] max-w-[780px]"
        x-data="{
            searchQuery: '',
            showOption(option) {
                return this.searchQuery === '' || option.toLowerCase().includes(this.searchQuery.toLowerCase());
            }
        }"
    >
        ...
        <?php if ($args['has_search_bar']) { ?>
            <input type='text' name='taxonomy-search' class="mb-6" placeholder="search..." x-model="searchQuery"></input>
        <?php } ?>
        ...
        <?php
            foreach((array) $args['labels'] as $label) {
                echo get_template_part('template-parts/filters/elements/checkbox', '', [
                    ...
                    'x-show' => $args['has_search_bar'] ? "showOption('" . str_replace(["'", '"'], '', $label) . "')" : '',
                    'is_array' => true,
                    'checked' => false,
                ]);
            }
        ...
        <button type="button" class="bg-navy float-right shadow-black-offset border-2 border-black hover:bg-yellow hover:text-black text-white font-sun-motter text-16 px-5 py-3 mt-8" x-on:click="<?php echo $args['x-show']; ?> = ! <?php echo $args['x-show']; ?>"> Update</button>
