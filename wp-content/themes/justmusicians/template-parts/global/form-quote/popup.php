<div data-popup="quote" class="popup-wrapper pt-28 md:pt-0 w-screen h-screen fixed top-0 left-0 z-50 flex items-center justify-center" x-show="showInquiryModal" x-cloak>
    ...
    <div data-trigger="quote" class="popup-close-bg bg-black/40 absolute top-0 left-0 w-full h-full cursor-pointer"
        x-on:click="if (currentInquirySlide == 'thankyou') { _showInquirySlide(''); } else { _showInquirySlide('discard'); }"
    ></div>
    ...
    <form x-ref="inquiryForm">
        <input type="hidden" name="inquiry_listing" x-ref="inquiryListing" />
        <?php echo get_template_part('template-parts/global/form-quote/date-slide', '', array()); ?>
        <?php echo get_template_part('template-parts/global/form-quote/location-slide', '', array()); ?>
        <?php echo get_template_part('template-parts/global/form-quote/details-slide', '', array()); ?>
        <?php echo get_template_part('template-parts/global/form-quote/email-slide', '', array()); ?>
        <?php echo get_template_part('template-parts/global/form-quote/discard-slide', '', array()); ?>
        <?php echo get_template_part('template-parts/global/form-quote/thank-you-slide', '', array()); ?>
    </form>
