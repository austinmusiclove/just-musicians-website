...
<div class="flex flex-col gap-4" x-show="!showEditForm" x-cloak>
    ...
    <div>
        ...
        <div class="flex gap-2 flex-col text-14 wysiwyg-content border border-black/20 p-2 rounded"
            :class="description ? '' : 'text-black/50'" x-html="description ? description : 'No description provided'">
        </div>
    </div>
    <!-- Requirements -->
    <div>
        <h3 class="font-bold text-16 mb-2">Requirements</h3>
            ...
            <template x-for="event in appEvents" :key="event.event_id">
                <div class="p-2 border border-black/20 rounded-sm">
                    <div class="flex flex-col">
                        ...
                        <span class="text-12 text-black/60"
                            x-text="event.start_date ? new Date(event.start_date + 'T00:00:00').toLocaleDateString('en', { month: 'short', day: 'numeric', year: 'numeric' }) : ''"></span>
                    </div>
                </div>
            </template>
        </div>
        <div class="text-16 text-black/50" x-show="appEvents.length === 0" x-cloak>No events associated with this
            application</div>
    </div>
    ...
    <div class="pt-4 flex gap-2 flex-wrap">
        ...
        <button type="button" x-on:click="showEditForm = true"
            class="bg-yellow hover:bg-navy text-black hover:text-white px-3 py-2 rounded-sm font-sun-motter text-14 w-fit whitespace-nowrap inline-block">
            Edit Application
        </button>
        <?php } ?>
        <?php if ($args['app_access'] === 'owner') { ?>
        ...
        <button type="button"
            class="bg-white hover:bg-red hover:text-white border border-black/20 hover:border-red px-3 py-2 rounded-sm font-sun-motter text-14 w-fit whitespace-nowrap inline-block"
            hx-delete="<?php echo site_url('/wp-html/v1/applications/' . get_the_ID()); ?>"
            hx-confirm="Are you sure you want to delete this application?" hx-target="#delete-result">
            Delete Application
