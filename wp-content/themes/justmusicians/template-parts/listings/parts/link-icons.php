...
<div class="flex items-center gap-1">
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['website']; ?>"                                             <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['website']) ? 'true' : 'false'; ?>"                <?php } ?>
        <?php if ($is_preview)  { ?> x-bind:href="pWebsite"                                                             <?php } ?>
        <?php if ($is_preview)  { ?> x-show="pWebsite" x-cloak                                                          <?php } ?>>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['instagram_url']; ?>"                                       <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['instagram_url']) ? 'true' : 'false'; ?>"          <?php } ?>
        <?php if ($is_preview)  { ?> x-bind:href="pInstagramUrl"                                                        <?php } ?>
        <?php if ($is_preview)  { ?> x-show="pInstagramUrl" x-cloak                                                     <?php } ?>>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['x_url']; ?>"                                               <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['x_url']) ? 'true' : 'false'; ?>"                  <?php } ?>
        <?php if ($is_preview)  { ?> x-bind:href="pXUrl"                                                                <?php } ?>
        <?php if ($is_preview)  { ?> x-show="pXUrl" x-cloak                                                             <?php } ?>>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['tiktok_url']; ?>"                                          <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['tiktok_url']) ? 'true' : 'false'; ?>"             <?php } ?>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['facebook_url']; ?>"                                        <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['facebook_url']) ? 'true' : 'false'; ?>"           <?php } ?>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['youtube_url']; ?>"                                         <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['youtube_url']) ? 'true' : 'false'; ?>"            <?php } ?>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['bandcamp_url']; ?>"                                        <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['bandcamp_url']) ? 'true' : 'false'; ?>"           <?php } ?>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['spotify_artist_url']; ?>"                                  <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['spotify_artist_url']) ? 'true' : 'false'; ?>"     <?php } ?>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['apple_music_artist_url']; ?>"                              <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['apple_music_artist_url']) ? 'true' : 'false'; ?>" <?php } ?>
    ...
    <a target="_blank"
        <?php if (!$is_preview) { ?> href="<?php echo $args['soundcloud_url']; ?>"                                      <?php } ?>
        <?php if (!$is_preview) { ?> x-show="<?php echo !empty($args['soundcloud_url']) ? 'true' : 'false'; ?>"         <?php } ?>
