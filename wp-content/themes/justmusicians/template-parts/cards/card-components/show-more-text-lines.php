...
<div x-data="{ expanded: false, isOverflowing: false }">
    ...
    <!-- x-init once when Alpine initializes this element. Detects whether the text is actually clipped
         by line-clamp-3 so the "Show more" button only appears when the content is truly truncated. -->
    <p class="whitespace-pre-wrap <?php echo "$text_color $text_size"; ?> line-clamp-3"
       x-init="$nextTick(() => {
           ...
           // $nextTick waits for Alpine to apply the :class binding (line-clamp-3) before measuring,
       ...
       :class="expanded ? '!line-clamp-none' : 'line-clamp-3'"><?php echo htmlspecialchars($text); ?></p>
