...
<button type="button"
    class="bg-yellow-light hover:bg-yellow px-3 py-2 rounded-sm font-sun-motter text-12 inline-block w-full"
    hx-target="this" hx-swap="outerHTML" hx-trigger="click"
    <?php // Get post url with alpine or regular var based on which one is passed in ?> <?php if ($listing_var) { ?>
    ...
    <?php } ?> <?php // Handle toasts ?> <?php if ($success_toast) { ?>
    x-init="$dispatch('success-toast', { 'message': '<?php echo $success_toast; ?>' })"
    <?php } else if ($error_toast) { ?>
    x-init="$dispatch('error-toast',   { 'message': '<?php echo $error_toast; ?>' })" <?php } ?>>
    <span class="flex justify-center" <?php if ($listing_var) { ?>
        x-bind:id="'request-proposal-btn-indicator-<?php echo $args['event_id']; ?>-' + <?php echo $listing_var; ?>"
        <?php } else { ?>
        id="request-proposal-btn-indicator-<?php echo $args['event_id'] . '-' . $args['listing_id']; ?>" <?php } ?>>
