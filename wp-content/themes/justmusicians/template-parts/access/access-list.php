...
<div id="<?php echo $container_id; ?>" hx-swap-oob="true">
    ...
    <ul class="mt-2 border border-black/10 rounded divide-y divide-black/10 max-h-48 overflow-y-auto">
        ...
        <li class="flex items-center justify-between gap-4 p-3 text-14">
            ...
            <span class="uppercase shrink-0 bg-yellow-20 text-yellow font-bold py-1 px-2">Owner</span>
            <?php } else {
            ...
            <div hx-post="<?php echo esc_url(site_url('/wp-html/v1/access/')); ?>"
                ...
                hx-target="#access-list-results-<?php echo esc_attr($args['subject_id']); ?>" hx-swap="innerHTML"
                hx-trigger="access-view-<?php echo (string) $entry->id; ?> from:body"
                hx-indicator="#<?php echo $container_id; ?>" hidden></div>
            <div hx-post="<?php echo esc_url(site_url('/wp-html/v1/access/')); ?>"
            ...
            <select class="border border-black/20 rounded-sm bg-white !text-14 !py-1.5 !pl-1 !pr-1 max-w-[80px]"
                onchange="htmx.trigger(document.body, 'access-' + this.value + '-<?php echo (string) $entry->id; ?>')">
                <option value="<?php echo esc_attr($entry->access_type); ?>" selected>
                    <?php echo esc_html(ucfirst($entry->access_type)); ?></option>
                <option value="<?php echo esc_attr($other_type); ?>"><?php echo esc_html(ucfirst($other_type)); ?>
                </option>
