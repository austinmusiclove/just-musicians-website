<div class="popup-wrapper pb-8 pt-28 md:pt-0 w-screen h-screen fixed top-0 left-0 z-50 flex items-center justify-center"
    x-show="showInquiryModal" x-cloak>
    ...
    <div class="popup-close-bg bg-black/40 absolute top-0 left-0 w-full h-full cursor-pointer"
        x-on:click="_tryExitInquiryModal()"></div>

    <div class="bg-white p-8 md:p-20 relative w-full h-full md:w-auto md:h-auto flex items-center justify-center"
        style="max-width: 780px;">

        <!-- Progress tracker -->
        <div class="progress-tracker bg-yellow h-2 absolute top-0 left-0 transition-all duration-500"
            x-bind:style="`width: ${inquiryProgress}%`"></div>
        ...
        <form x-ref="inquiryForm" hx-post="/wp-html/v1/events/" hx-target="#inquiry-result" hx-ext="disable-element"
            hx-disable-element=".inquiry-submit-button" x-on:keydown.enter="
            ...
            <?php echo get_template_part('template-parts/inquiries/popup-slides/request-slide',         '', []); ?>
