<div class="flex flex-col gap-4" x-show="!showEditForm">
    ...
    <div class="pt-4 flex gap-2">
        <button type="button" x-on:click="showEditForm = true"
            class="bg-yellow hover:bg-navy text-black hover:text-white px-6 py-3 rounded-sm font-sun-motter text-14 w-fit whitespace-nowrap inline-block">
            Edit Event
        </button>
        <button type="button"
            class="bg-white hover:bg-red hover:text-white border border-black/20 hover:border-red px-6 py-3 rounded-sm font-sun-motter text-14 w-fit whitespace-nowrap inline-block"
            hx-delete="<?php echo site_url('/wp-html/v1/events/' . get_the_ID()); ?>"
            hx-confirm="Are you sure you want to delete this event?" hx-target="#delete-result">
            Delete Event
