...
class LeafletMap {
    ...
    for (let i = 0; i < markers.length; i++) {
      ...
      if (this.enablePopups) {
        let popupContent = `
                ...
                `;
        let popup = L.popup().setLatLng([latitude, longitude]).setContent(popupContent);
  ...
  getCluster() {
    return L.markerClusterGroup({
      ...
      maxClusterRadius: 40,
      spiderfyPolylineOptions: {
        weight: 1.5,
        color: '#442A88',
        opacity: 0.5
      },
      // FCECAC FFE77F EE7E7E EB4C67 B1A6CE 442A88
...
class VenuePageManager {
  ...
  getVenueReviews(evnt) {
    ...
    this.getVenueReviewsFromServer(venueId).then(response => {
      ...
      // parse data
      let reviewsHtml = '';
      let payStructureChartData = {
        'Guarantee': 0,
        'Door Deal': 0,
        'Bar Deal': 0,
        'Tips': 0
      };
      let payMethodChartData = {
        'Cash': 0,
        'Check': 0,
        'Direct Deposit': 0,
        'Zelle': 0,
